...
class ICRARewards:
    ...
    def _reward_alive(self):
        return self.env.commands[:, 0]*0.5 + 0.2
    ...
    def _reward_loco_energy(self):
        return torch.sum(
            ...
            , dim=1)
        # return torch.sum((self.torques[:,:self.num_actions_loco]*self.dof_vel[:,:self.num_actions_loco]).clip(max=0.), dim=1)

    def _reward_manip_energy(self):
        return torch.sum(
            torch.abs(self.env.torques[:, self.env.num_actions_loco:]*self.env.dof_vel[:, self.env.num_actions_loco:])
            , dim=1)
        # return torch.sum((self.env.torques[:,self.env.num_actions_loco:]*self.env.dof_vel[:,self.env.num_actions_loco:]).clip(max=0.), dim=1)
    ...
    def _reward_penal_vel_y(self):
        return torch.sum(torch.square(self.env.base_lin_vel[:, 1].unsqueeze(-1)), dim=1)
